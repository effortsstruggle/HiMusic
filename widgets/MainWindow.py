from PyQt5.QtCore import (Qt, QSize, QPoint, QMetaObject, pyqtSlot, QUrl, QTimer, QDir)
from PyQt5.QtWidgets import (QApplication, QMainWindow, QLabel, QSlider,
                             QListView,QTableWidgetItem,
...
from widgets import ui_MainWindow, TrayIconWidget, VolumeControler, SubThread, GlobalVar
import json
import qtawesome as qta #图标集库
...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.ui.more.setIcon(qta.icon('fa.bars', color=color)) #更多按钮
    ...
    def setup_cache(self):
        # 初始化“上次在听”
        try:
            datas = GlobalVar.helper.cache['last_song']
        ...
        # 初始化“上次的播放列表”
        try:
            datas = GlobalVar.helper.cache['last_playlist']
            if not datas:
                raise Exception()
        ...
        # 初始化“我的收藏”
        datas = GlobalVar.helper.cache['collections']
    ...
    def setup_settings(self):
        settings = GlobalVar.helper.settings
        # QSS
        qss = GlobalVar.helper.qss
        ...
        index = GlobalVar.helper.settings['lrcs'].setdefault('distance', 1)
        for s, v in heights:
    ...
    def setup_engine(self):
        for index, engine in enumerate(GlobalVar.helper.engines.values()):
            self.ui.input_engine.addItem(engine.name, engine)
            self.ui.settings_default_engine.addItem(engine.name, engine.pre)

            if engine.pre == GlobalVar.helper.default_engine.pre:
    ...
    def setup_system_tray(self):
        self.menu_widget = TrayIconWidget.CTrayIconWidget(self)
    ...
    def setup_menus(self):
        # 音量菜单
        self.volume_widget = VolumeControler.CVolumeControler(self)
...
''')
        ...
        # 获取音乐
        music_content_getter = SubThread.SubThread(task=SubThread.SubThread.get_music_content, data=url)
    ...
    def search(self):
        ...
        # 搜索
        result_getter = SubThread.SubThread(task=SubThread.SubThread.get_result, keyword=keyword)
...
'''
    ...
    def closeEvent(self, event):
        # 设置
        GlobalVar.helper.settings['general']['close_trayicon'] = self.ui.settings_close_trayicon.isChecked()  # 关闭时最小化到托盘
        GlobalVar.helper.settings['general']['default_engine'] = self.ui.settings_default_engine.currentData()  # 默认搜索引擎
        GlobalVar.helper.settings['download']['path'] = self.ui.settings_download_path_edit.text()  # 下载地址
        GlobalVar.helper.settings['lrcs']['distance'] = self.ui.settings_lrcs_distance.currentIndex()  # 歌词高度

        GlobalVar.helper.save_settings()  # 保存设置

        # 缓存
        try:
            GlobalVar.helper.cache['last_song'] = self.__playlist[self.playlist.currentIndex()][1]  # 上次在听
        except:
            GlobalVar.helper.cache['last_song'] = []

        try:
            GlobalVar.helper.cache['last_playlist'] = list(zip(*self.__playlist))[1]
        except:
            GlobalVar.helper.cache['last_playlist'] = []

        GlobalVar.helper.cache['collections'] = []
        for row in range(self.ui.collections_view.rowCount()):
            GlobalVar.helper.cache['collections'].append(self.ui.collections_view.get_datas(row))

        GlobalVar.helper.save_cache()  # 保存缓存
    ...
    def add_media(self, datas, local=False, play=False) -> int:
        ...
        if engine == 'local':
            ...
            # 网络音乐
            music_url_getter = SubThread.SubThread(task=SubThread.SubThread.get_music_url, data=sign)
    ...
    def insert_songs_datas(self, widget, datas, row):
        ...
        if engine == 'local':
            url = ':'.join(data)
            img_getter = SubThread.SubThread(task=SubThread.SubThread.get_local_pixmap, url=url)
        else:
            img_getter = SubThread.SubThread(task=SubThread.SubThread.get_pixmap, data=datas[4])
...
''')
    ...
    def refresh_songsui(self, datas):
        ...
        if engine == 'local':
            url = ':'.join(data)
            pixmap_getter = SubThread.SubThread(task=SubThread.SubThread.get_local_pixmap, url=url)
            img_getter = SubThread.SubThread(task=SubThread.SubThread.get_local_img, url=url)
            lrc_getter = SubThread.SubThread(task=SubThread.SubThread.get_local_lrcs, url=url)
        else:
            pixmap_getter = SubThread.SubThread(task=SubThread.SubThread.get_pixmap, data=pixmap)
            img_getter = SubThread.SubThread(task=SubThread.SubThread.get_img, data=bigimg)
            lrc_getter = SubThread.SubThread(task=SubThread.SubThread.get_lrcs, data=sign)
...
#lrcbox[lrcbox_index="%s"] {
    ...
    def quit_app(self):
        json.dump(GlobalVar.helper.settings, open('settings.json', 'w'), indent=4)